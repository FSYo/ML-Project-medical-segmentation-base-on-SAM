            ...
            for tr_data in data["validation"] :
                ...
                for i in range(100, num_slices):
                    ...
                    for l in range(1, 13) : 
                        ...
                        if(sumX == 0) : 
                            continue
                        sumX /= np.sum(label_data[:, :, i] == l)
                        sumY /= np.sum(label_data[:, :, i] == l)
                        num = num + 1
                        

                        input_point = np.array([[sumX, sumY]])
                        ...
                        plt.axis('off')
                        plt.savefig('epoch'+str(epoch)+'slice'+str(i)+'-'+str(l)+'.png')

                        mask_threshold = 0.0
                        # answer = (batched_output[0]['masks'][T] > mask_threshold).cpu().numpy()
                        answer = masks[0]
                        print("shape of answer and label and info : ", label.shape, answer.shape, np.sum(label * answer), np.sum(label) , np.sum(answer) ) 

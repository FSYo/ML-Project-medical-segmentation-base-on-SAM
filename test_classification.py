...
from utils.dataset import to_uint8_rgb, remove_pure_background, to_tensor
...
def main():
    ...
    sam = sam_model_registry['default']('./models/fin_sam_vit_h_4b8939.pth')
    model = SamBTCV(sam, requires_classification=True)
    model.load_state_dict(torch.load('./models/cls/best_model.pth'))
    model.to(device)
    ...
    for data in tqdm(dataset_info['validation'], desc = "Validating data"):
        images = nib.load(os.path.join('./dataset', data['img'])).get_fdata()
        labels = nib.load(os.path.join('./dataset', data['label'])).get_fdata()
        ...
        for image in tqdm(images, desc = "img in data"):
...
def predict_labels(output, iou_thresh : float = 0.5):
    ...
    index = torch.argsort(iou_predictions, descending=True)
    iou_predictions = iou_predictions[index]
    class_predictions = torch.argmax(class_predictions[index], dim=1)
    masks = masks[index]
